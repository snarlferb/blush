...
- [ ] Buffering refers to how the shell handles the input before it's executed. Typically, when you paste multiple lines of code or commands into a shell with buffering enabled, it retains those lines in an input buffer, allowing you to edit or review them before submitting them for execution. We might consider calling our readline, "editline" if it turns into a library. Otherwise it'll just be miscellaneous files of no distinct kind.
