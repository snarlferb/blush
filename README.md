...
- [ ] So in other words, you can keep the traditional Vi mode, wherein its disriminated in such a way where it doesnt exist simultaneously as a "Panel", or you can have them enabled/disabled simultaneously (as in an X/O Panel you simultaneously flip, changing that particular layer of binding targets) An error should occur if `setsw -o inspanel` without it also set to a particular `<Key>`, and the same isnt the case for `set -o vi`, which would be preserving the traditional.
