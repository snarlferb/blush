...
- [ ] Please bare with me, as ive officially fried my brain while trying to put this in order. You can use `set -o vi` which lets you have it enabled in the same way as traditional command shells. Otherwise, you'd have (`setsw -o npanel` or `set -o <allpanels>`, whichever way it ends up being), which is effectively `setsw -/+ o` (enable/disable) but from a key binding. This implies you can do the same with Insert Mode, so you can emulate the traditional feel—And wherein it can be set to whichever key (`<inspanel> : "\033OP"` or `"\033OP": "sw_to_inspanel"` or whatever it ends up looking like)
- [ ] So in other words, you can keep the traditional Vi mode, wherein its disriminated in such a way where it doesnt exist simultaneously as a "Panel", or you can have them enabled/disabled simultaneously (as in an X/O Panel you simultaneously flip, changing that particular layer of binding targets) An error should occur if `setsw -o inspanel` without it also set to a particular `<Key>`, and the same isnt the case for `set -o inspanel`, which would be preserving the traditional.
- [ ] Things like the kill buffer will be configurable as well; Theres quite a few things that im changing or introducing...
