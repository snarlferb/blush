...
- [ ] You can use `setsw -o npanel` to enable it. You would still be able to use `set -o npanel` which lets you have it enabled in the same way as it would be in traditional command shells, effectively the ability to run `set -/+ o` but from a key binding. This implies you can do the same with Insert Mode, so you can emulate the traditional feel—And wherein it can be set to whichever key (`<inspanel> : "\033OP"` or `"\033OP": "sw_to_inspanel"` or whatever it ends up looking like)
- [ ] So in other words, you can have them enabled/disabled simultaneously (as in an X/O Panel you simultaneously flip, changing that particular layer of binding targets) or you can have them discriminated, so that they dont exist, atleast not from the keyboard. An error should occur if `setsw -o inspanel` without it also set to a particular `<Key>`, and the same isnt the case for `set -o inspanel`, which would be preserving the traditional... Okay my brain is fried from attempting to wrap my mind around it, haha.
