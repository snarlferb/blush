...
- [ ] **Panels**: Why would you want to toggle normal mode from a shell that is suppose to be for running commands directly? Instead the normal mode should be a simplification upon commands, that is, a constant layer that you have the options of customizing yourself, like `Transformed<command>` = `_B_`... The transformed commands being in the interface for setting each simplification—which is for those people who dont like the default ViMode (and it would presumably be found in `~/.npanelrc`)  Two other Panels are Emacs and MiMode, which may be activated upon pressing Fn (mod) key; i'm still playing with this idea.
